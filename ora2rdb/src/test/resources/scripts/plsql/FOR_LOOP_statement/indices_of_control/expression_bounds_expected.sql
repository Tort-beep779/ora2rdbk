/* Процедура */
CREATE GLOBAL TEMPORARY TABLE NT1_P_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT2_P_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT1_MULTUNION_NT2_P_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE PROCEDURE P_Expr_Bounds
AS
  DECLARE reslt INTEGER = 0; 
  /* TYPE nested_typ IS TABLE OF varchar2(20); */
  /* nt1 nested_typ := nested_typ('альфа','бета','гамма'); */
  /* nt2 nested_typ := nested_typ('бета','гамма','альфа'); */
  DECLARE i_FOR1 INTEGER;
BEGIN
   INSERT INTO NT1_P_Expr_Bounds(VAL) VALUES ('alfa');
   INSERT INTO NT1_P_Expr_Bounds(VAL) VALUES ('betta');
   INSERT INTO NT1_P_Expr_Bounds(VAL) VALUES ('gamma');
   INSERT INTO NT2_P_Expr_Bounds(VAL) VALUES ('betta');
   INSERT INTO NT2_P_Expr_Bounds(VAL) VALUES ('gamma');
   INSERT INTO NT2_P_Expr_Bounds(VAL) VALUES ('alfa');
   INSERT INTO NT1_MULTUNION_NT2_P_Expr_Bounds (VAL)
   SELECT VAL FROM NT1_P_Expr_Bounds UNION ALL SELECT VAL FROM NT2_P_Expr_Bounds;
   FOR SELECT ID FROM NT1_MULTUNION_NT2_P_Expr_Bounds
   ORDER BY ID ASC
   INTO :i_FOR1
   DO
   BEGIN
     reslt = :reslt + i_FOR1; 
   END
END; 

/* Функция */
CREATE GLOBAL TEMPORARY TABLE NT1_F_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT2_F_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT1_MULTUNION_NT2_F_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE FUNCTION F_Expr_Bounds
RETURNS varchar(100)
AS
  DECLARE reslt INTEGER = 0; 
  /* TYPE nested_typ IS TABLE OF varchar2(20); */
  /* nt1 nested_typ := nested_typ('альфа','бета','гамма'); */
  /* nt2 nested_typ := nested_typ('бета','гамма','альфа'); */
  DECLARE i_FOR1 INTEGER;
BEGIN
   INSERT INTO NT1_F_Expr_Bounds(VAL) VALUES ('alfa');
   INSERT INTO NT1_F_Expr_Bounds(VAL) VALUES ('betta');
   INSERT INTO NT1_F_Expr_Bounds(VAL) VALUES ('gamma');
   INSERT INTO NT2_F_Expr_Bounds(VAL) VALUES ('betta');
   INSERT INTO NT2_F_Expr_Bounds(VAL) VALUES ('gamma');
   INSERT INTO NT2_F_Expr_Bounds(VAL) VALUES ('alfa');
   INSERT INTO NT1_MULTUNION_NT2_F_Expr_Bounds (VAL)
   SELECT VAL FROM NT1_F_Expr_Bounds UNION ALL SELECT VAL FROM NT2_F_Expr_Bounds;
   FOR SELECT ID FROM NT1_MULTUNION_NT2_F_Expr_Bounds
   ORDER BY ID ASC
   INTO :i_FOR1
   DO
   BEGIN
     reslt = :reslt + i_FOR1; 
   END
   RETURN reslt; 
END; 

/* Пакет */
CREATE GLOBAL TEMPORARY TABLE NT1_Pack_Expr_Bounds_PF_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT2_Pack_Expr_Bounds_PF_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT1_MULTUNION_NT2_Pack_Expr_Bounds_PF_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT1_Pack_Expr_Bounds_PP_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT2_Pack_Expr_Bounds_PP_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT1_MULTUNION_NT2_Pack_Expr_Bounds_PP_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE OR ALTER PACKAGE Pack_Expr_Bounds
AS BEGIN 
   FUNCTION PF_Expr_Bounds
   RETURNS varchar(100);
   PROCEDURE PP_Expr_Bounds; 
END; 

/* Тело пакета */
RECREATE PACKAGE BODY Pack_Expr_Bounds
AS BEGIN
   FUNCTION PF_Expr_Bounds
   RETURNS varchar(100)
   AS
     DECLARE reslt INTEGER = 0; 
     /* TYPE nested_typ IS TABLE OF varchar2(20); */
     /* nt1 nested_typ := nested_typ('альфа','бета','гамма'); */
     /* nt2 nested_typ := nested_typ('бета','гамма','альфа'); */
     DECLARE i_FOR1 INTEGER;
   BEGIN
     INSERT INTO NT1_Pack_Expr_Bounds_PF_Expr_Bounds(VAL) VALUES ('alfa');
     INSERT INTO NT1_Pack_Expr_Bounds_PF_Expr_Bounds(VAL) VALUES ('betta');
     INSERT INTO NT1_Pack_Expr_Bounds_PF_Expr_Bounds(VAL) VALUES ('gamma');
     INSERT INTO NT2_Pack_Expr_Bounds_PF_Expr_Bounds(VAL) VALUES ('betta');
     INSERT INTO NT2_Pack_Expr_Bounds_PF_Expr_Bounds(VAL) VALUES ('gamma');
     INSERT INTO NT2_Pack_Expr_Bounds_PF_Expr_Bounds(VAL) VALUES ('alfa');
     INSERT INTO NT1_MULTUNION_NT2_Pack_Expr_Bounds_PF_Expr_Bounds (VAL)
     SELECT VAL FROM NT1_Pack_Expr_Bounds_PF_Expr_Bounds UNION ALL SELECT VAL FROM NT2_Pack_Expr_Bounds_PF_Expr_Bounds;
     FOR SELECT ID FROM NT1_MULTUNION_NT2_Pack_Expr_Bounds_PF_Expr_Bounds
     ORDER BY ID ASC
     INTO :i_FOR1
     DO
     BEGIN
       reslt = :reslt + i_FOR1; 
     END
     RETURN reslt;
   END

   PROCEDURE PP_Expr_Bounds
   AS
     DECLARE reslt INTEGER = 0; 
     /* TYPE nested_typ IS TABLE OF varchar2(20); */
     /* nt1 nested_typ := nested_typ('альфа','бета','гамма'); */
     /* nt2 nested_typ := nested_typ('бета','гамма','альфа'); */
     DECLARE i_FOR1 INTEGER;
   BEGIN
     INSERT INTO NT1_Pack_Expr_Bounds_PP_Expr_Bounds(VAL) VALUES ('alfa');
     INSERT INTO NT1_Pack_Expr_Bounds_PP_Expr_Bounds(VAL) VALUES ('betta');
     INSERT INTO NT1_Pack_Expr_Bounds_PP_Expr_Bounds(VAL) VALUES ('gamma');
     INSERT INTO NT2_Pack_Expr_Bounds_PP_Expr_Bounds(VAL) VALUES ('betta');
     INSERT INTO NT2_Pack_Expr_Bounds_PP_Expr_Bounds(VAL) VALUES ('gamma');
     INSERT INTO NT2_Pack_Expr_Bounds_PP_Expr_Bounds(VAL) VALUES ('alfa');
     INSERT INTO NT1_MULTUNION_NT2_Pack_Expr_Bounds_PP_Expr_Bounds (VAL)
     SELECT VAL FROM NT1_Pack_Expr_Bounds_PP_Expr_Bounds UNION ALL SELECT VAL FROM NT2_Pack_Expr_Bounds_PP_Expr_Bounds;
     FOR SELECT ID FROM NT1_MULTUNION_NT2_Pack_Expr_Bounds_PP_Expr_Bounds
     ORDER BY ID ASC
     INTO :i_FOR1
     DO
     BEGIN
       reslt = :reslt + i_FOR1; 
     END
   END
END; 

/* Триггер DML */
CREATE GLOBAL TEMPORARY TABLE NT1_T_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT2_T_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT1_MULTUNION_NT2_T_Expr_Bounds (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE OR ALTER TRIGGER T_Expr_Bounds
  AFTER INSERT 
  ON students
AS
  DECLARE reslt INTEGER = 0; 
  /* TYPE nested_typ IS TABLE OF varchar2(20); */
  /* nt1 nested_typ := nested_typ('альфа','бета','гамма'); */
  /* nt2 nested_typ := nested_typ('бета','гамма','альфа'); */
  DECLARE i_FOR1 INTEGER;
BEGIN
   INSERT INTO NT1_T_Expr_Bounds(VAL) VALUES ('alfa');
   INSERT INTO NT1_T_Expr_Bounds(VAL) VALUES ('betta');
   INSERT INTO NT1_T_Expr_Bounds(VAL) VALUES ('gamma');
   INSERT INTO NT2_T_Expr_Bounds(VAL) VALUES ('betta');
   INSERT INTO NT2_T_Expr_Bounds(VAL) VALUES ('gamma');
   INSERT INTO NT2_T_Expr_Bounds(VAL) VALUES ('alfa');
   INSERT INTO NT1_MULTUNION_NT2_T_Expr_Bounds (VAL)
   SELECT VAL FROM NT1_T_Expr_Bounds UNION ALL SELECT VAL FROM NT2_T_Expr_Bounds;
   FOR SELECT ID FROM NT1_MULTUNION_NT2_T_Expr_Bounds
   ORDER BY ID ASC
   INTO :i_FOR1
   DO
   BEGIN
     reslt = :reslt + i_FOR1; 
   END
END;

/* Анонимный блок */
CREATE GLOBAL TEMPORARY TABLE NT1_NestTable_1 (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT2_NestTable_1 (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

CREATE GLOBAL TEMPORARY TABLE NT1_MULTUNION_NT2_NestTable_1 (
    ID  BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
    VAL VARCHAR(20)
);

EXECUTE BLOCK 
AS 
  DECLARE reslt INTEGER = 0; 
  /* TYPE nested_typ IS TABLE OF varchar2(20); */
  /* nt1 nested_typ := nested_typ('альфа','бета','гамма'); */
  /* nt2 nested_typ := nested_typ('бета','гамма','альфа'); */
  DECLARE i_FOR1 INTEGER;
BEGIN
   INSERT INTO NT1_NestTable_1(VAL) VALUES ('alfa');
   INSERT INTO NT1_NestTable_1(VAL) VALUES ('betta');
   INSERT INTO NT1_NestTable_1(VAL) VALUES ('gamma');
   INSERT INTO NT2_NestTable_1(VAL) VALUES ('betta');
   INSERT INTO NT2_NestTable_1(VAL) VALUES ('gamma');
   INSERT INTO NT2_NestTable_1(VAL) VALUES ('alfa');
   INSERT INTO NT1_MULTUNION_NT2_NestTable_1 (VAL)
   SELECT VAL FROM NT1_NestTable_1 UNION ALL SELECT VAL FROM NT2_NestTable_1;
   FOR SELECT ID FROM NT1_MULTUNION_NT2_NestTable_1
   ORDER BY ID ASC
   INTO :i_FOR1
   DO
   BEGIN
     reslt = :reslt + i_FOR1; 
   END
END;

