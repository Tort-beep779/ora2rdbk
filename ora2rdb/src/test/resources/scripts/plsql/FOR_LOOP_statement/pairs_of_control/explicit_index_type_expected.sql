/* Процедура */
CREATE GLOBAL TEMPORARY TABLE VEC_P_Explicit_Index (
    K INTEGER,
    VAL INTEGER,
    CONSTRAINT PK_VEC_P_Explicit_Index PRIMARY KEY (K)
);
...
BEGIN
  UPDATE OR INSERT INTO VEC_P_Explicit_Index VALUES (3, 10);
  UPDATE OR INSERT INTO VEC_P_Explicit_Index VALUES (1, 11);
  UPDATE OR INSERT INTO VEC_P_Explicit_Index VALUES (100, 34);
  FOR SELECT K, VAL FROM VEC_P_Explicit_Index
  ORDER BY K ASC
  INTO :n_FOR1, :m_FOR1
  DO
  BEGIN
...
AS BEGIN
   ...
   BEGIN
     ...
     FOR SELECT K, VAL FROM VEC_Pack_Explicit_Index_PF_Explicit_Index
     ORDER BY K ASC
     INTO :n_FOR1, :m_FOR1
     DO
     BEGIN
   ...
   BEGIN
     ...
     FOR SELECT K, VAL FROM VEC_Pack_Explicit_Index_PP_Explicit_Index
     ORDER BY K ASC
     INTO :n_FOR1, :m_FOR1
     DO
     BEGIN
...
BEGIN
  ...
  FOR SELECT K, VAL FROM VEC_T_Explicit_Index
  ORDER BY K ASC
  INTO :n_FOR1, :m_FOR1
  DO
  BEGIN
